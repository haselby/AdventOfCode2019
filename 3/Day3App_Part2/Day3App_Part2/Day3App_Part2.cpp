...
//Dictionary - Global
static std::map<std::pair<int, int>, int> PathOfWires;


void compute(char* array_ptr, size_t array_length, int instruction_pointer, int x_loc, int y_loc, int wireId) {
	...
	if (array_ptr[instruction_pointer] == 'R') {
		...
		for (size_t i = 0; i < move; i++) {
			x_loc++;
			PathOfWires[std::make_pair(x_loc, y_loc)] = (PathOfWires[std::make_pair(x_loc, y_loc)] != wireId ? (PathOfWires[std::make_pair(x_loc, y_loc)] + wireId) : PathOfWires[std::make_pair(x_loc, y_loc)]);
		}
	}
	...
	if (deliminatorLocation == 0) {
		...
		compute(array_ptr, array_length, deliminatorLocation + 1, x_loc, y_loc, wireId);
	}
...
int main()
{
	...
	compute(wire1, sizeof(wire1) / sizeof(char), 0, 0, 0, 1);
	compute(wire2, sizeof(wire2) / sizeof(char), 0, 0, 0, 2);
	...
	for (auto elem : PathOfWires)
	{
		if (elem.second > 2) {
			...
			std::cout << elem.first.first << "," << elem.first.second << " " << elem.second << " " << "MahattanDistance: " << MahattanDistance << "\n";
