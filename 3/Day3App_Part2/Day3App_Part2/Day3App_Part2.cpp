// Day3App_Part2.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
void compute(char* array_ptr, size_t array_length, int instruction_pointer, std::map<std::pair<int, int>, int> &PathOfWire, int x_loc, int y_loc, int stepCounter) {

	int deliminatorLocation{ 0 };
	for (size_t i = instruction_pointer; i < array_length; i++) {
		if (array_ptr[i] == ',') {
			deliminatorLocation = i;
			break;
		}
	}

	std::string strMove;
	strMove.assign(array_ptr + instruction_pointer + 1, array_ptr + (deliminatorLocation != 0 ? deliminatorLocation : array_length - 1));
	std::stringstream ss_move(strMove);
	int move{};
	ss_move >> move;


	if (array_ptr[instruction_pointer] == 'R') {
		//Move Right
		for (size_t i = 0; i < move; i++) {
			x_loc++;
			stepCounter++;
			PathOfWire[std::make_pair(x_loc, y_loc)] = (PathOfWire[std::make_pair(x_loc, y_loc)] == 0 ? stepCounter : PathOfWire[std::make_pair(x_loc, y_loc)]);
		}
	}
	else if (array_ptr[instruction_pointer] == 'U') {
		//Move Up
		for (size_t i = 0; i < move; i++) {
			y_loc++;
			stepCounter++;
	...
	else if (array_ptr[instruction_pointer] == 'L') {
		//Move Left
		for (size_t i = 0; i < move; i++) {
			x_loc--;
			stepCounter++;
	...
	else if (array_ptr[instruction_pointer] == 'D') {
		//Move Down
		for (size_t i = 0; i < move; i++) {
			y_loc--;
			stepCounter++;
	...
	std::cout << "(x,y) = " << x_loc << "," << y_loc << "\n";

	if (deliminatorLocation == 0) {

		return;
	}
	else {
		compute(array_ptr, array_length, deliminatorLocation + 1, PathOfWire, x_loc, y_loc, stepCounter);
	}
}
